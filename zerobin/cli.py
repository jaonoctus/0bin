...
def infos():
    """ Print the version and the directories used by this 0bin instance. """

    ensure_app_context()
    print(f"Zerobin version: {zerobin.__version__}")
    print(f"Data dir (pastes and counter): {settings.DATA_DIR}")
    print(
        f"Config dir (config file, secret key and custom views): {settings.CONFIG_DIR}"
    )
    print(
...
def main():
    ...
    clize.run(runserver, delete_paste, infos, clean_expired_pastes)
